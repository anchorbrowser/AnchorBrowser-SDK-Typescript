...
export class Anchorbrowser {
   ...
   */
  constructor({
    baseURL = readEnv('ANCHORBROWSER_BASE_URL'),
    apiKey = readEnv('ANCHORBROWSER_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AnchorbrowserError(
        "The ANCHORBROWSER_API_KEY environment variable is missing or empty; either provide it, or instantiate the Anchorbrowser client with an apiKey option, like new Anchorbrowser({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.anchorbrowser.io`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Anchorbrowser.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ANCHORBROWSER_LOG'), "process.env['ANCHORBROWSER_LOG']", this) ??
      defaultLogLevel;
  ...
  protected authHeaders(opts: FinalRequestOptions): NullableHeaders | undefined {
    return buildHeaders([{ 'anchor-api-key': this.apiKey }]);
  }
